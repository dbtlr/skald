...
### Fixed
- `sk config init` now detects and offers API providers. Previously the picker only listed CLI providers (those with a binary in `PATH`), so `codex` and `anthropic` never appeared, and `sk config init --provider codex` was wrongly rejected as unknown. API providers are now detected by their credentials — `codex` via the Codex ChatGPT-subscription login, `anthropic` via `ANTHROPIC_API_KEY`.
